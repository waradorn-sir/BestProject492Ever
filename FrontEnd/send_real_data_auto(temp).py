...
def send_packet_real(packet,key): # for send real packet
    ...
    response = requests.request("POST", "http://10.81.65.136:5000/send", headers={'Content-Type': 'application/json'}, json=packet) #send data to server
    print(response.text) # show response from server


def write_old_data(rawdata): # write new data from sensors to log sensor file
    with open(f'./OldDataTemp.json') as datafile: # load data before append
        fett = json.load(datafile) # store data in fett(variable)
        fett.append(rawdata) # append new data to fett(variable)
    with open(f'./OldDataTemp.json', "w") as outfile: # load file in write mode
